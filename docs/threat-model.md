...
2. **Desktop → vault.** The scanner is allowed to read only the selected root
...
3. **Desktop → VPS.** SSH host identity is pinned. Local processes use fixed
   argv with `shell: false`; remote actions and SFTP paths are validated.
4. **VPS runtime → OpenAI.** The runtime makes outbound HTTPS polls using a
   restricted runtime key. The VPS exposes no inbound MCP port.
5. **ChatGPT → notes.** Prompts, search queries, note text, and links are
   untrusted content. MCP results are data, never policy or executable commands.
6. **VPS host → container.** The VPS/Docker operator can inspect plaintext
...
| SSH MITM or host replacement | first-use fingerprint confirmation and app-private pin; drift hard-fails | first-use verification is owner responsibility |
...
| Sensitive diagnostics | bounded redacted journal; no raw stdout/stderr, paths, keys, queries, or note text | local crash/core dumps need OS protection |
